...
MYDETAILS = {
    "JobUrl" : "https://careers.adobe.com/us/en/apply?jobSeqNo=ADOBUSR146367EXTERNALENUS&utm_medium=phenom-feeds&source=LinkedIn&utm_source=linkedin&step=1",
    "name": "Syed Amim",
...
chrome_binary_path = MYDETAILS["machine"]["chrome_binary_path"]  # Update this path if necessary
...
def add_date_field_value(field_id, date_value):
    try:
        ...
        print(f"Error setting date for field {field_id}: {e}")
...
def close_overlay():
    try:
        overlay_close_button = WebDriverWait(driver, 5).until(
            EC.element_to_be_clickable((By.XPATH, "//button[contains(@class, 'close-overlay') or contains(@class, 'overlay-close')]"))
        )
...
try:
    ...
    add_date_field_value("experienceData[0].fromTo.startDate", "03/2023")
    select_dropdown_by_id("educationData[0].degree", "Bachelors")
    add_date_field_value("educationData[0].fromTo.startDate", "07/2018")
    add_date_field_value("educationData[0].fromTo.endDate", "05/2022")
    add_form_field_value("socialMediaAccountDataV2[0].socialNetworkAccountURL", MYDETAILS["linkedin"])
    click_button_by_id("next")
    ...
    # Page 5
    click_button_by_class_name("btn primary-button btn-submit")
